...
sys.path.insert(0, os.path.abspath('../../dryxPyramid/dryxPyramid'))
...
html_theme_options = {
    ...
    'github_repo': 'dryxPyramid',
    'strap_line': "basic, reusable code for pyramid webapps"
}
...
""" % locals()
link_resolver_url = "https://github.com/thespacedoctor/dryxPyramid/tree/master"

# General information about the project.
now = datetime.now()
now = now.strftime("%Y")
project = u'dryxPyramid'
...
exclude_patterns = ['_build', '_templates',
                    '**__version__.py', '**setup.py', 'api/dryxPyramid.rst']
modindex_common_prefix = ["dryxPyramid."]

# (source start file, target name, title, author, documentclass [howto/manual]).
latex_documents = [
    ('index', 'dryxPyramid.tex', u'dryxPyramid Documentation',
     u'Dave Young', 'manual'),
]
...
def updateUsageMd():
    ...
    *Grab the usage from cl_utils.py to display in README.md*
    """
    from dryxPyramid import cl_utils
...
def generateAutosummaryIndex():
    ...
    # GET ALL SUBPACKAGES
    allSubpackages = ["dryxPyramid"]
    allSubpackages += findAllSubpackges(
        pathToPackage="dryxPyramid"
    )
